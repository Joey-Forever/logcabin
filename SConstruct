...
def Protobuf(env, source):
    # Variant directories use duplicate=0, so protoc reads the source-tree
    # .proto file directly and writes generated files under build/.
    source_node = env.File(source).srcnode()
    source_base = os.path.splitext(source_node.path)[0]
    cc = env.File('#build/' + source_base + '.pb.cc')
    targets = [
        cc,
        env.File('#build/' + source_base + '.pb.h'),
        env.File('#build/' + source_base + '_pb2.py'),
    ]
    env.Command(targets,
                source_node,
                '$PROTOC -I. $PROTOCFLAGS '
                '--cpp_out=${PROTOCCPPOUTFLAGS}build '
                '--python_out=build $SOURCE')
...
Export('env')
SConscript('Core/SConscript', variant_dir='build/Core', duplicate=0)
SConscript('Event/SConscript', variant_dir='build/Event', duplicate=0)
SConscript('RPC/SConscript', variant_dir='build/RPC', duplicate=0)
SConscript('Protocol/SConscript', variant_dir='build/Protocol', duplicate=0)
SConscript('Tree/SConscript', variant_dir='build/Tree', duplicate=0)
SConscript('Client/SConscript', variant_dir='build/Client', duplicate=0)
SConscript('Storage/SConscript', variant_dir='build/Storage', duplicate=0)
SConscript('Server/SConscript', variant_dir='build/Server', duplicate=0)
